...
public class AppTest {
    ...
    String email = "jassika@yahoo.com";
    int userId = TestUtils.getUserIdByEmail(mockMvc, email);

    PersonDto personDto = new PersonDto();
    personDto.setFirstName("personNew");
    personDto.setLastName("NewPerson");
    personDto.setEmail("queue@gmail.com");
    mockMvc.perform(patch("/people/{id}", userId).contentType(MediaType.APPLICATION_JSON)
    ...
    assertNotNull(repository.findByEmail("queue@gmail.com"));
    ...
    public void deletePerson() throws Exception {
        String userEmaile = "jassika@yahoo.com";
        int userId = TestUtils.getUserIdByEmail(mockMvc, userEmaile);
        MockHttpServletResponse response = mockMvc.perform(delete("/people/{id}", userId)).andReturn().getResponse();

        assertThat(response.getStatus()).isEqualTo(200);
        assertNull(repository.findByEmail(userEmaile));
    }
    // END
}
